...
export default function Prospects() {
  ...
  useEffect(() => {
    let mounted = true;
    setLoading(true);
    const fetcher = campaignFilter
      ? api.getCampaignProspects(campaignFilter)
      : api.getAllProspects();
    fetcher
      .then(data => { if (mounted) setProspects(data); })
  ...
  const openDrawer = async (p) => {
    ...
    try {
      const detail = await api.getProspect(p.id);
      setDrawerData(detail);
    } catch { setDrawerData(p); }
  ...
  const handleSort = (field) => {
    setSortField(f => f === field ? f : field);
